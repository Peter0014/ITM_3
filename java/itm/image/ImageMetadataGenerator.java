...
public class ImageMetadataGenerator 
{
    ...
    */
    protected ImageMedia processImage( File input, File output, boolean overwrite ) throws IOException, Exception, IllegalArgumentException
    {
        if ( ! input.exists() ) {
            throw new IOException( "Input file " + input + " was not found!" );
        }
        ...
        { // load the input image
            ...
            media.addTag(input.getName().substring(input.getName().indexOf('.')+1,input.getName().length()));
            
            // set orientation
            media.setOriantation((media.getHeight() < media.getWidth()?
            		ImageMedia.ORIENTATION_LANDSCAPE : ImageMedia.ORIENTATION_PORTRAIT));
            ...
            if(cm != null)
            {
            	...
            	media.setNumOfImgComp(cm.getNumComponents());
            	media.setNumOfImgColComp(cm.getNumColorComponents());
            }
